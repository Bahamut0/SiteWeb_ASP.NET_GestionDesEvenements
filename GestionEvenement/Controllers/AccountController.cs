        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    //{
                    //    using (ApplicationDbContext dbUsers = new ApplicationDbContext())
                    //    {
                            
                    //        var _connectedUser = dbUsers.Users.SingleOrDefault(x => x.Email == model.Email);
                    //        var _userRole = dbUsers.Roles.Select(x => x.Users.FirstOrDefault(y => y.UserId == _connectedUser.Id)).FirstOrDefault();
                    //        var role = dbUsers.Roles.Where(x => x.Id == _userRole.RoleId).FirstOrDefault(); 
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //await _roleManager.CreateAsync(new IdentityRole("Modérateur"));

                    //2-lier un role à l'utilisateur créé
                    await UserManager.AddToRoleAsync(user.Id, "Adhérent");
                    await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);


                    using (GestionEvenementEntities db = new GestionEvenementEntities())
                    {
                        ...
                        db.Users.Add(new Users()
                        {
                            ...
                            IdRole = 3,// le role 3 est le role adherent dans notre base
                            IdTrancheAge= 7, //id 7 est la tranche age "tout âge" dans notre base
                            Photo = "~/Image/inconnu.png"
